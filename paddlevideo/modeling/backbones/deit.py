...
class Block(nn.Layer):
    ...
    def forward(self, x, B, T, W):
        ...
        if self.attention_type in ['space_only', 'joint_space_time']:
            ...
            x = xt
            x = paddle.concat(
                (init_cls_token, init_dist_token, x), axis=1) + paddle.concat(
                    (cls_token, dist_token, res), axis=1)
...
class PatchEmbed(nn.Layer):
    ...
    def forward(self, x):
        ...
        W = x.shape[-1]
        x = x.flatten(2).transpose((0, 2, 1))  # [BT,nHnW,F]
        return x, T, W


@BACKBONES.register()
class DistilledVisionTransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        self.dist_token = self.create_parameter(shape=(1, 1, embed_dim),
                                                default_initializer=zeros_)
        self.pos_embed = self.create_parameter(shape=(1, num_patches + 2,
                                                      embed_dim),
                                               default_initializer=zeros_)
        self.pos_drop = nn.Dropout(p=drop_rate)

        if self.attention_type != 'space_only':
            self.time_embed = self.create_parameter(shape=(1, seg_num,
                                                           embed_dim),
                                                    default_initializer=zeros_)
    ...
    def forward_features(self, x):
        B = x.shape[0]
        # B = paddle.shape(x)[0]
        x, T, W = self.patch_embed(x)  # [BT,nHnW,F]
        cls_tokens = self.cls_token.expand(
            (x.shape[0] if B > 0 else 3 * T, -1, -1))  # [BT, 1, F]
        dist_tokens = self.dist_token.expand(
            (x.shape[0] if B > 0 else 3 * T, -1, -1))
        x = paddle.concat((cls_tokens, dist_tokens, x), axis=1)  # [BT,2+nHnW,F]
        ...
        x = self.pos_drop(x)  # [BT,2+nHnW,F]

        # Time Embeddings
        if self.attention_type != 'space_only':
            # cls_tokens = x[:B, 0, :].unsqueeze(1)
            cls_tokens = x[:B, 0, :].unsqueeze(1) if B > 0 else x.split(
                T)[0].index_select(paddle.to_tensor([0]), axis=1)  # [B, F]
            # dist_tokens = x[B:2B, 0, :].unsqueeze(1)
            dist_tokens = x[:B, 1, :].unsqueeze(1) if B > 0 else x.split(
                T)[0].index_select(paddle.to_tensor([1]), axis=1)  # [B, F]
            x = x[:, 2:]  # [BT,nHnW,F]
            _bt, _n, _m = x.shape
            _b = B
            ...
            x = paddle.concat((cls_tokens, dist_tokens, x),
                              axis=1)  # [BT,2+nHnW,F]
